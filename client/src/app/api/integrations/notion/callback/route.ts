...
export async function GET(req: NextRequest) {
  ...
  const { projectId, slug, sessionId = `notion:${projectId}`, userId, userName } = state;
  ...
  try {
    ...
    console.error("Notion Dynamic MCP Callback Error:", err);
    return NextResponse.redirect(
      `${integrationsUrl}?error=${encodeURIComponent(err.message || "Notion OAuth connection error")}`
    );
  }
}
