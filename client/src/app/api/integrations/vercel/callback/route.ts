...
export async function GET(req: NextRequest) {
  ...
  const { projectId, slug, sessionId = `vercel:${projectId}`, userId, userName } = state;
  ...
  try {
    ...
    return NextResponse.redirect(`${integrationsUrl}?connected=vercel`);
  } catch (err: any) {
    console.error("Vercel Dynamic MCP Callback Error:", err);
    return NextResponse.redirect(
      `${integrationsUrl}?error=${encodeURIComponent(err.message || "Vercel OAuth connection error")}`
    );
  }
}
