...
export async function GET(req: NextRequest) {
  ...
  const { projectId, slug, sessionId = `sentry:${projectId}`, userId, userName } = state;
  ...
  try {
    ...
    return NextResponse.redirect(`${integrationsUrl}?connected=sentry`);
  } catch (err: any) {
    console.error("Sentry Dynamic MCP Callback Error:", err);
    return NextResponse.redirect(
      `${integrationsUrl}?error=${encodeURIComponent(err.message || "Sentry OAuth connection error")}`
    );
  }
}
