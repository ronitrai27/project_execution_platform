...
import { getHistory, stopAgent } from "./action";

interface UseAgentStateCallbacks<
  TAgentState extends object | WithMessages,
  TInterruptValue,
> {
  /** Callback for when a checkpoint starts.*/
  onCheckpointStart?: (
    checkpoint: AppCheckpoint<TAgentState, TInterruptValue>,
  ) => void;
...
>(callbacks?: UseAgentStateCallbacks<TAgentState, TInterruptValue>) {
  const { onCheckpointStart, onCheckpointEnd, onCheckpointStateUpdate } =
    callbacks ?? {};
  ...
  async function fork(agentInput: ForkAgentInput<TAgentState>) {
    removeAppCheckpointsAfterCheckpoint(
      agentInput.config.configurable.checkpoint_id,
    );
  ...
  function removeAppCheckpointsAfterCheckpoint(checkpointId: string) {
    const index = appCheckpoints.findIndex(
      (node) =>
        node.checkpointConfig.configurable.checkpoint_id === checkpointId,
    );
    if (index !== -1) {
  ...
  async function callAgent(
    ...
    try {
      ...
      const messageStream = callAgentRoute<
        TAgentState,
        TInterruptValue,
        TResumeValue
      >(agentInput);
  ...
  async function restore(threadId: string) {
    ...
    try {
      ...
      for (let i = 0; i < history.length; i++) {
        if (
          skipToCheckpointId &&
          history[i].config.configurable.checkpoint_id !== skipToCheckpointId
        ) {
          continue;
        }
        ...
        skipToCheckpointId =
          history[i].parent_config?.configurable.checkpoint_id;
      }
  ...
  function processHistoryCheckpoint(
    ...
    if (appCheckpoints.length > 0) {
      ...
      lastCheckpoint.stateDiff = getStateDiff(
        lastCheckpoint.stateInitial,
        checkpoint.values,
      );
      ...
      // Keep interrupt value for historical view
      interruptionInLastCheckpoint =
  ...
  function createAppCheckpoint(
    checkpoint: Checkpoint<TAgentState, TInterruptValue>,
  ): AppCheckpoint<TAgentState, TInterruptValue> {
    return {
      nodes: checkpoint.next.map((x, index) => {
        const matchingKey = Object.keys(checkpoint.metadata?.writes ?? {}).find(
          (key) => key === x,
        );
        const value = matchingKey
          ? checkpoint.metadata?.writes?.[matchingKey]
          : undefined;
        return {
          name: x,
          state: matchingKey
  ...
  function updateGraphNodeStateFromMetadata(
    appCheckpoint: AppCheckpoint<TAgentState, TInterruptValue>,
    checkpoint: Checkpoint<TAgentState, TInterruptValue>,
  ) {
    // Update nodes states with the writes from the checkpoint metadata
    Object.entries(checkpoint.metadata?.writes ?? {}).forEach(
      ([key, value]) => {
        const matchingNodes = appCheckpoint.nodes.filter(
          (node) => node.name === key,
        );
  ...
  function processMessageChunk(
    ...
    const message = lastCheckpoint.state.messages.find(
